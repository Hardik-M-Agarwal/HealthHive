...
const DocumentsView = () => {
  ...
  const handleToggleFavorite = async (id) => {
    try {
      ...
      setDocuments(prev => prev.map(doc => doc._id === id ? { ...doc, isFavorite: !doc.isFavorite } : doc));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-100">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-semibold text-slate-900">Medical Documents</h2>
            <p className="text-sm text-slate-500 mt-1">Your central health repository</p>
          </div>
          <button
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <select
            ...
            className="px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
          >
            {categories.map(cat => (
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg border transition-colors flex items-center gap-2 font-medium ${
              showFavorites
          ...
          >
            <svg
              className={`w-5 h-5 ${showFavorites ? 'fill-current text-yellow-400' : 'text-slate-400'}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {activeSearch && (
            <div className="flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-700 rounded-full border border-blue-100">
              <span className="text-sm">"{activeSearch}"</span>
              <button onClick={handleClearSearch} className="hover:text-blue-900 transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
      ...
      {documents.length > 0 ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-slate-100">
          ...
          <p className="text-slate-500 text-sm mb-5">Upload your first medical document</p>
          <button
            onClick={() => setShowUploadModal(true)}
